...
function Header() {
  ...
  const HEADERHAM = (
    <header
      className={`${DataPlicityFONT.className} sticky top-0 z-100 w-full text-black p-4 backgroundColors backdrop-blur-sm`}
    >
      <div className="flex justify-between gap-15  items-center">
        <Link
          href={"/"}
          className="hover:scale-110 transition-all duration-200 delay-100 flex items-center"
        >
          <Image
            src={"/logo.png"}
            width={120}
            height={120}
            alt="Logo de Dataplicity"
      ...
      <div className="flex justify-between gap-15 px-[10%] items-center">
        ...
        <nav className="navbar flex gap-5 font-bold font-mono">
          <Link
            ...
            className={
              pathname === "/"
                ? "text-white  CurrentLink nav-link"
                : "notCurrentLink nav-link"
            }
          >
            Inicio
          </Link>
